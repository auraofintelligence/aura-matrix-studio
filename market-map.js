...
export function mountMarket({page,screen,headless=false}){
 ...
 async function open(mode='Map',filters={}){
  ...
  try{const [L,loaded]=await Promise.all([loadMap(),data||fetch('assets/market-data.json?v=0.4.12').then(r=>{if(!r.ok)throw Error('Places could not load. Close and try again.');return r.json();})]);if(disposed||!dialog.open||token!==opening)return;data=loaded;
