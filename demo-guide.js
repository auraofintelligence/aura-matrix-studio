...
export function mountGuideEntry({screen,page,go}){
  ...
  const b=button('How the demo works',()=>go(DEMO_GUIDE),'demo-guide-link');const icon=make('img');icon.src='assets/mockplus/aura-guide.svg';icon.alt='';b.prepend(icon);const links=make('div','demo-entry-links'),guide=make('a','demo-guide-link','Matrix guide');guide.href='guide.html';links.append(b,guide);entry.append(links);screen.append(entry);
}
export function mountDemoGuide({screen,go}){
  ...
  const vector=section('Exploring Vector Space','The neutral arrow field is a demonstration, not an actual embedding vector field. Move your mouse or one finger to steer the arrows. Drag with the mouse or two fingers to orbit; pinch to zoom. The corner axes show orientation. The seven colour toggles show or hide tori; choose a chakra in the selection menu to pick its facets. Try example, then Analyse, calculates word-pattern vectors from labelled text. These use shared words, not a neural model understanding their meaning.');link(vector,'Open Vector Space','aura-vector-space');vector.append(make('p','','Inspect a memory to edit its colour, direction, charge or pulse, and link it to geometry. Earth map shows latitude and longitude. Save / restore exports this lab separately.'));
