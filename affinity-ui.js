import {MARKET_PAGES,MARKET_CATEGORIES,filterMarket,mountMarket} from './market-map.js?v=0.4.13';
...
export function mountAffinity({page,screen,go}){
 ...
 async function search(){
  ...
  try{if(!data){const response=await fetch('assets/market-data.json?v=0.4.13');if(!response.ok)throw Error('Listings could not load. Open Search to retry.');data=await response.json();}if(!disposed&&panel.contains(results))draw();}catch(e){status.textContent=e.message;}
 }
