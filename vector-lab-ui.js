...
export function mountVectorLab({screen,go}){
  ...
  const stage=make('div','vl-stage'),canvas=make('canvas');canvas.setAttribute('aria-label','Shared torus, geosphere and memory volume. Move one finger to steer the demonstration field; two fingers rotate and zoom. Choose a chakra to select its facets.');stage.append(canvas);
  ...
  for(const [v,t]of [['memories','Memories'],...SHELLS.map(([n],i)=>[String(i),n+' facets']),['geosphere','Earth map']]){const o=make('option','',t);o.value=v;mode.append(o);}mode.onchange=()=>{if(scene){scene.target=mode.value;if(/^[0-6]$/.test(mode.value)){scene.setShellVisible(Number(mode.value),true);syncShells();}scene.render();}drawScene();};
  ...
  const shellBar=make('div','vl-shells');shellBar.setAttribute('role','group');shellBar.setAttribute('aria-label','Visible chakra tori');
  const shellButtons=SHELLS.map(([name,colour],index)=>{const b=button('',()=>{if(!scene)return;const visible=!scene.shellVisibility[index];scene.setShellVisible(index,visible);if(!visible&&mode.value===String(index)){mode.value='memories';scene.target='memories';address=null;scene.address=null;}syncShells();drawScene();},'vl-shell-toggle');b.style.setProperty('--shell-colour',colour);b.setAttribute('aria-label','Show '+name+' torus');b.title=name+' torus';b.setAttribute('aria-pressed','true');b.append(make('span','',''));shellBar.append(b);return b;});stage.append(shellBar);
  function syncShells(){shellButtons.forEach((b,i)=>b.setAttribute('aria-pressed',String(scene.shellVisibility[i])));}
  ...
  const demoNote=make('p','vl-demo-note','Demonstration field. Not an actual embedding vector field.');stage.append(demoNote);
  ...
  function drawScene(){const r=current(),hasVectors=state.records.some(r=>r.values.length);scene?.update(state.records,r,nearest(state.records,r));demoNote.hidden=hasVectors||mode.value==='geosphere';overlay.textContent=address?addressLabel(address):r?r.title:state.records.length?`${state.records.length} memories · ${hasVectors?'word-pattern space':'awaiting analysis'}`:mode.value==='geosphere'?'Earth · latitude / longitude':'Demonstration arrow field in the cubic volume';hint.textContent=mode.value==='geosphere'?'Earth · Natural Earth land map · tap for latitude / longitude':/^[0-6]$/.test(mode.value)?'Tap a facet to select; drag to orbit; pinch to zoom':!hasVectors?'Move mouse / finger to steer; mouse drag / two fingers orbit; pinch zoom':state.records.length>600?'Showing first 600 points + selection · find every record in Inspect':'Drag to orbit · pinch to zoom · tap to select';}
