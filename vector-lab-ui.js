...
import {emptyLab,parseImport,mergeImport,record,exampleRecords,validateLab,nearest,ALGORITHMS,COLOURS} from './vector-lab-data.js?v=0.4.22';
import {VectorLabScene} from './vector-lab-scene.js?v=0.4.22';
import {openLabStore} from './vector-lab-store.js?v=0.4.22';
...
export function mountVectorLab({screen,go}){
  ...
  const demoNote=make('p','vl-demo-note','Demonstration cloud. Not an actual embedding vector field.'),demoTip=make('p','vl-demo-tip');demoTip.hidden=true;demoTip.setAttribute('role','status');stage.append(demoNote,demoTip);
  ...
  function drawScene(){const r=current(),hasVectors=state.records.some(r=>r.values.length);scene?.update(state.records,r,nearest(state.records,r));demoNote.hidden=hasVectors||mode.value==='geosphere';demoTip.hidden=true;overlay.textContent=address?addressLabel(address):r?r.title:state.records.length?`${state.records.length} memories · ${hasVectors?'word-pattern space':'awaiting analysis'}`:mode.value==='geosphere'?'Earth · latitude / longitude':'Demonstration points in the cubic volume';hint.textContent=mode.value==='geosphere'?'Earth · Natural Earth land map · tap for latitude / longitude':!hasVectors?'Hover or tap a point · drag to orbit · pinch to zoom':state.records.length>600?'Showing first 600 points + selection · find every record in Inspect':'Drag to orbit · pinch to zoom · tap to select';}
  ...
  function drawPanel(){
    ...
    if(tab==='data'){
      ...
      const row=make('div','vl-row'),run=button(worker?'Analysing…':'Analyse '+state.records.length+' memories',()=>{
        if(!state.records.length)return;stop();worker=new Worker(new URL('./vector-lab-worker.js?v=0.4.22',import.meta.url),{type:'module'});const job=worker;report('Calculating vectors and positions on this device…');drawPanel();
  ...
  try{scene=new VectorLabScene(canvas,pick=>{if('demo' in pick){demoTip.hidden=pick.demo===null;demoTip.textContent=pick.demo===null?'':`Demonstration point ${pick.demo}. Illustrative position only; no embedding or personal data.`;}else if(pick.record)select(pick.record);else{address=pick.address;scene.address=address;tab='inspect';drawPanel();drawScene();}});}catch(e){canvas.hidden=true;stage.append(make('p','vl-no-webgl','3D is unavailable on this device. Import, analysis and the searchable inspector still work.'));for(const b of [side,mode,rays,play,fit])b.disabled=true;}
