import {loadMap} from './market-map.js?v=0.4.15';
import {EARTH,EARTH_WIDE,earthRows,filterEarth,clusterEarth,pickEarthMarker,personalEarthRows,importEarthPoints,saveEarthPoints} from './earth-data.js?v=0.4.17';
import {readTravelProject,writeTravelProject,CELESTIAL} from './travel-data.js?v=0.4.17';
import {CROWN} from './original-routes.js?v=0.4.15';
...
function earthSnapshot(){return snapshot||(snapshot=fetch('assets/earth/manifest.json?v=0.4.15').then(async response=>{if(!response.ok)throw Error('Earth layer list could not load.');const metadata=await response.json();const data=await Promise.all(metadata.layers.map(async layer=>{const r=await fetch(layer.data);if(!r.ok)throw Error(layer.label+' could not load.');return earthRows(layer,await r.json());}));return {metadata,records:data.flat()};}).catch(e=>{snapshot=null;throw e;}));}
