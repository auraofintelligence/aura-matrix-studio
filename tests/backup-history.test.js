...
test('extra pages register once under their intended parent',()=>{const source={pages:[]};addExtraPages(source);addExtraPages(source);assert.equal(source.pages.length,4);assert.equal(source.pages.find(p=>p.id===MERCH).parent,'9635446B-1F60-4AF2-A62C-E40C90E1806E');assert.ok(source.pages.find(p=>p.id===DATA_TRANSFER));});
