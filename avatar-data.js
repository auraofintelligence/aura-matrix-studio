import {validateProject} from './core.js?v=0.4.0';
...
export const AVATAR_SECTIONS=[
 ...
 {id:'EB351F52-5B46-44B7-8014-8F7193AA2DAA',key:'space',title:'Personal space',summary:'Comfortable distances and social boundaries',icon:'space',intro:'Record what feels comfortable to you. These are preferences, not automatic proximity alerts.',dataset:'boundaries',steps:[{title:'One-to-one space',fields:[measure('space-close','Close contact distance'),measure('space-conversation','Conversation distance')],help:'Distance from you to the other person. Leave a distance blank if it depends on the situation.'},{title:'Group and public space',fields:[measure('space-group','Group conversation distance'),measure('space-public','Public setting distance')]},{title:'Context and boundaries',fields:[note('space-context','Where or with whom does this apply?'),note('space-contact','Touch and greeting preferences'),note('space-adjustments','What helps you feel comfortable?')]}]},
 {id:'C6AFC90C-3BCB-4CFE-A0A8-2A59A175ED46',key:'eyes',title:'Eyes and spacing',summary:'Optional eye spacing for your avatar',icon:'eyes',intro:'Use measurements you already know, or leave them blank. No photos are required.',dataset:'body',steps:[{title:'Eye measurements',fields:[measure('eyes-spacing','Distance between pupil centres','mm'),measure('eyes-left','Left eye width, corner to corner','mm'),measure('eyes-right','Right eye width, corner to corner','mm')],help:'For avatar proportions only. These values do not provide a glasses prescription.'}]},
...
export function avatarProgress(project,section){const v=avatarValues(project,section),fields=fieldsFor(section);return {filled:fields.filter(f=>v[f.id]!== '').length,total:fields.length};}
