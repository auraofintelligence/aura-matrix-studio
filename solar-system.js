...
const DAY=86400000;
export function zoomSolarView(camera,factor,point={x:108,y:87}){const zoom=Math.max(.5,Math.min(32,camera.zoom*factor)),ratio=zoom/camera.zoom;return {zoom,x:point.x-(point.x-camera.x)*ratio,y:point.y-(point.y-camera.y)*ratio};}
...
export function solarSystemSvg(state,tracks,mode='compact',selected='Earth',camera={zoom:1,x:0,y:0}){
 ...
 return `<svg viewBox="0 0 336 180" role="img" aria-label="Solar system at ${state.date}: Sun, eight planets and Earth's Moon; viewed from above the north ecliptic pole"><rect width="336" height="180" rx="7" fill="#101c32"/><defs><clipPath id="solar-map-clip"><rect width="214" height="165"/></clipPath></defs><g clip-path="url(#solar-map-clip)"><g data-solar-world transform="translate(${camera.x} ${camera.y}) scale(${camera.zoom})">${tracks.filter(t=>planets.some(p=>p.name===t.name)).map(t=>`<path d="${path(t.points)}" fill="none" stroke="${t.name===selected?'#7b9dbd':'#31415b'}" stroke-width=".7"/>`).join('')}<circle cx="108" cy="87" r="6" fill="#ffd567"/><text x="108" y="104" text-anchor="middle" font-size="9" fill="#ffd567">Sun</text>${moon}${markers}</g></g>${legend}<circle cx="225" cy="129" r="2" fill="white"/><text x="235" y="133" font-size="10" fill="white">Moon (enlarged)</text><text x="220" y="150" font-size="10" fill="white">${selected}</text><text x="220" y="163" font-size="9" fill="#b9c8dc">${caption}</text><text x="108" y="175" text-anchor="middle" font-size="8" fill="#b9c8dc">${mode==='compact'?'Distances compressed logarithmically':'Projected distances to scale'} · markers enlarged</text></svg>`;
}
