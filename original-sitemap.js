import {HOME,livePage,canonicalPage,CAMERA_VARIANTS} from './original-routes.js?v=0.3.3';
...
export function pageTitle(page){const id=canonicalPage(page.id),live=livePage(id),name=page.name.replace(/ CK$/,'');return live?(live.shape==='flat'?'Finite map':['Red','Orange','Yellow','Green','Blue','Indigo','Violet'][live.shell]+' torus')+' · '+(live.face==='I'?'inside':'outside'):LABELS[name]||name;}
export function mapPages(pages,section='daily',query=''){
  const clean=pages.filter(p=>!CAMERA_VARIANTS[p.id]&&p.name!=='Page'),q=query.trim().toLowerCase();
  ...
  const byId=new Map(pages.map(p=>[p.id,p]));return clean.filter(p=>{let current=p;const seen=new Set();while(current&&!seen.has(current.id)){seen.add(current.id);if(ROOTS[section]?.includes(current.name))return true;current=byId.get(current.parent);}return false;});
}
export function mountSiteMap({page,screen,pages,go,previewMode=false}){
  ...
  const icons=new Map();function collect(cs){for(const c of cs){if(c.properties.URL)for(const link of c.links)if(!icons.has(canonicalPage(link.target)))icons.set(canonicalPage(link.target),c.properties.URL);collect(c.children);}}all.forEach(p=>collect(p.controls));
  ...
  function draw(){
    ...
    list.replaceChildren();for(const p of found.slice(offset,offset+12)){
      ...
      const icon=make('span','wayfinder-icon');const asset=icons.get(p.id);if(asset&&p.name!=='Learning'){const image=make('img');image.src='assets/mockplus/'+asset;image.alt='';icon.append(image);}else icon.textContent=SYMBOLS[p.name]||'◇';link.append(icon,make('span','wayfinder-label',pageTitle(p)));
